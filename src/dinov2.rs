// Vendored from candle-transformers, patched for non-square inputs.
...
impl Module for PatchEmbed {
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
        ...
        if h % PATCH_SIZE != 0 || w % PATCH_SIZE != 0 {
            candle::bail!("image size {h}x{w} is not a multiple of patch size {PATCH_SIZE}")
        }
...
impl DinoVisionTransformer {
    ...
    fn interpolate_pos_encoding(&self, xs: &Tensor, img_h: usize, img_w: usize) -> Result<Tensor> {
        let npatch = xs.dim(1)? - 1;
        let n = self.pos_embed.dim(1)? - 1;
        let sqrt_n = (n as f64).sqrt() as usize;
        if npatch == n && img_h == img_w {
        ...
        let (grid_h, grid_w) = (img_h / PATCH_SIZE, img_w / PATCH_SIZE);
        let patch_pos_embed = patch_pos_embed
            .reshape((1, sqrt_n, sqrt_n, dim))?
            .transpose(2, 3)?
            .transpose(1, 2)?;
        // Original uses bicubic; nearest is what candle offers.
        let patch_pos_embed = patch_pos_embed.upsample_nearest2d(grid_h, grid_w)?;
    ...
    fn prepare_tokens_with_mask(&self, xs: &Tensor) -> Result<Tensor> {
        let (_b, _c, img_h, img_w) = xs.dims4()?;
        let xs = self.patch_embed.forward(xs)?;
        let xs = Tensor::cat(&[&self.cls_token, &xs], 1)?;
        &xs + &self.interpolate_pos_encoding(&xs, img_h, img_w)?
    }
    ...
    pub fn get_intermediate_layers(
        ...
        let outputs = output
            .iter()
            .map(|out| Ok(self.norm.forward(out)?.i((.., 1..))?))
            .collect::<Result<Vec<_>>>()?;
